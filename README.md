...
- 为角色绑定本地参考音频，或先由 LLM 根据角色音色与上下文生成 26 至 32 个字符、优先为 30 个字符的可编辑参考文案，再使用确认后的音色描述和参考文案通过 Qwen / MiMo 生成参考音色；多次点击“生成音色”会按点击顺序逐项执行，每项完成后立即更新页面与音色库；系统以 7 至 10 秒作为这类动态参考音频的工程目标。
- 页面启动时使用 Bing 每日图片作为背景：当天已缓存的图片 URL 会直接复用，跨天才请求新图；该纯展示缓存不进入工程文件、`UnitaleDB` 或导出内容。
...
对于上表中的官方默认端口，WebUI 会根据 `Base URL` 自动校正协议：`8300` 使用 `indextts2`，`8301` 至 `8306` 使用 `reference-text-clone`。这也会修复旧浏览器配置曾把非 `8300` 服务补成 IndexTTS2、进而漏传 `prompt_text` 的情况；其他端口仍采用配置中手动选择的协议。
...
- 每日背景使用独立键 `storyforge_bing_daily_background_date` 与 `storyforge_bing_daily_background_url`；只存当天日期和已校验的 Bing 图片 URL。Bing 原始元数据接口不允许静态浏览器页面直接跨域读取，因此前端经兼容中转获取原始 Bing 图片 URL；中转不可用时使用本地渐变回退。
